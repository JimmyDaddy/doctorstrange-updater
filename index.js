...
class DoctorStrangeUpdater {
  ...
  downLoadPatch = () => {
    ...
    FS.copyFile(SOURCE_ROOT + '/doctor.zip', tempDataFile).then((value) => {
      ...
      const ret = FS.downloadFile({
        fromUrl: this.downloadHost + '?patchId=' + this.patchId + '&bundleId=' + this.BUNDLEID + '&platform=' + platform,
  ...
  downLoadData = () => {
    ...
    const platform = Platform.OS === 'ios' ? 1 : 2
    const ret = FS.downloadFile({
      fromUrl: this.downloadHost + '?version=' + this.newVersion + '&bundleId=' + this.BUNDLEID + '&platform=' + platform,
      toFile: downloadDestPath,
      begin,
      progress,
      background: true,
